...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	for (int astro = 0; astro < 3; astro++)
	{
		...
		case 0:
		{
			theAsteroids[astro]->setSpritePos({ ( 185, 200) });
		}
		break;

		case 1:
		{
			theAsteroids[astro]->setSpritePos({ (370, 200) });
		}
		break;

		case 2:
		{
			theAsteroids[astro]->setSpritePos({ (555, 200) });
		}
		break;
		...
		theAsteroids[astro]->setSpriteTranslation({ 0, 0 });
